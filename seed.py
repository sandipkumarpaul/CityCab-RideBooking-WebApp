"""Demo dataset for CityCab.

Run `python seed.py` to reset the local database. The app also calls
`seed_database()` automatically the first time it starts with an empty database.
"""
from models import db, User, DriverProfile, TrustedContact, Ride, Payment, Review, SOSAlert

def seed_database():
    """Drop all tables and recreate them with demo users, rides and SOS alerts.
...
if __name__ == '__main__':
    ...
    with app.app_context():
        seed_database()
    print('CityCab demo database seeded.')
